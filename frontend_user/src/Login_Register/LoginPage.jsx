...
import { useNavigate } from 'react-router-dom'; // <<< THÊM MỚI: Import useNavigate
...
const LoginPage = ({ onClose }) => {
    const { login } = useAuth();
    const navigate = useNavigate(); // <<< THÊM MỚI: Khởi tạo hook navigate
    ...
    // --- HÀM XỬ LÝ ĐĂNG NHẬP (ĐÃ SỬA ĐỔI PHẦN ĐIỀU HƯỚNG) ---
    const handleLogin = async () => {
        if (loading) return;
        setLoading(true);
        setError("");
        ...
        try {
            ...
            if (response.status === 200 && response.data.token && response.data.user) {
                ...
                setEmail("");
                setPassword("");
                onClose(); // <<< Đóng modal ngay trước khi điều hướng


                // --- SỬA ĐỔI: Logic điều hướng dựa trên role ---
                if (userData.role === 'admin') {
                    console.log("Saving admin token to localStorage and redirecting...");
                    // *** LƯU TOKEN VÀO localStorage CHO TRANG ADMIN HTML ĐỌC ***
                    localStorage.setItem('adminToken', token);
                    // (Tùy chọn) Lưu thêm tên nếu không muốn decode token ở trang HTML
                    // localStorage.setItem('adminName', userData.name);
    
                    // Chuyển hướng trình duyệt sang trang HTML admin bằng replace
                    const adminHomePageUrl = '/frontend_admin/adminPage/HomePage.html'; // Đảm bảo đường dẫn đúng
                    window.location.replace(adminHomePageUrl);
                    // onClose(); // Không cần thiết vì trang đã chuyển hướng
    
                } else { // Customer
                    // Chỉ đóng modal và điều hướng trong React App
    ...
    // --- PHẦN JSX (KHÔNG THAY ĐỔI CẤU TRÚC) ---
    return (
        <div
            className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            onClick={onClose}
